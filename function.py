...
def generar_reporte_shop_name_final(df):
    ...
    columnas = ['Online Store', 'Active Stores', 'GMV', 'complete_order_cnt',
                'pay_order_cnt', 'b_duty_cancel_order_cnt', 'cancel_order_cnt',
                'r_burn', 'b2c_total', 'p2c_total']
    for col in columnas:
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)

    # Online Connection Rate como número
    df['Online Connection Rate'] = pd.to_numeric(df['Online Connection Rate'].str.replace('%', ''), errors='coerce')

    # Agrupar por shop_name, brand_name y semana
    ...
    agg = agg[['shop_name', 'brand_name', 'week_number',
               'Online Store', 'Active Stores', 'GMV',
               'complete_order_cnt', 'pay_order_cnt',
               'Completion rate', 'ticket_promedio',
               'B-cancel rate', 'r_burn', 'b2c_total',
               'p2c_total', 'online rate %']]

    # Reorganizar a formato deseado
    df_melt = agg.melt(id_vars=['shop_name', 'brand_name', 'week_number'], var_name='Metric', value_name='Value')
    df_pivot = df_melt.pivot_table(index=['shop_name', 'brand_name', 'Metric'], columns='week_number', values='Value').reset_index()
    ...
    # Alerta si más del 40% de métricas tienen WoW negativa por tienda → se marca a todas las métricas
    alerta_dict = (
        df_pivot
        .groupby('shop_name')['WoW']
        .apply(lambda x: (x < 0).mean() > 0.4)
        .to_dict()
    )
...
def generar_reporte_all_brand_final(df):
    ...
    agg = df.groupby(['all_brand', 'week_number']).agg({
        'Online Store': 'sum',
        'Active Stores': 'sum',
        'GMV': 'sum',
        'complete_order_cnt': 'sum',
    ...
    df_melt = agg.melt(id_vars=['all_brand', 'week_number'], var_name='Metric', value_name='Value')
    df_pivot = df_melt.pivot_table(index=['all_brand', 'Metric'], columns='week_number', values='Value').reset_index()
